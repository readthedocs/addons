...
export function injectNonLatestVersionWarning(config) {
  ...
  if (true) {
    // (highest && highest !== config.version.slug) {
    let admonition = `
<div class="admonition warning">
  ...
  <p>
    ...
    <a class="reference" href="${window.location.protocol}//${window.location.hostname}/${config.project.language}/${config.version.slug}/">v.3.2.1</a>.
  </p>
</div>
`;
    ...
    let main = document.querySelector(
      config.features.non_latest_version_warning.query_selector
    );
