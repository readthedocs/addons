...
export class FlyoutElement extends LitElement {
  static elementName = "readthedocs-new-flyout";
  ...
  loadConfig(config) {
    ...
    // accordingly
    if (!NewFlyoutAddon.isEnabled(config)) {
      return;
    }
    this.config = config;
  ...
  renderIcon() {
    library.add(faInfo);
    const iconInfo = icon(faInfo);
    return html`<span>${iconInfo.node[0]}</span>`
  }
