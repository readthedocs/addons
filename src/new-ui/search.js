...
export class SearchElement extends LitElement {
  static elementName = "readthedocs-new-search";
  ...
  loadConfig(config) {
    ...
    // accordingly
    if (!NewSearchAddon.isEnabled(config)) {
      return;
    }
  ...
  renderIcon() {
    return html`<span>${this.inputIcon.node[0]}</span>`;
  }
  ...
  getRecentSearches() {
    ...
    const recentSearches = NewSearchAddon.getLocalStorage()[localStorageKey];
  ...
  storeRecentSearch(block, result) {
    ...
    NewSearchAddon.setLocalStorage(recentSearchesObject);
  }
